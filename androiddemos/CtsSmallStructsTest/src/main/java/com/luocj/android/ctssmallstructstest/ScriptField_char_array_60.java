 ...
 * @hide
 */
public class ScriptField_char_array_60 extends Script.FieldBase {
    ...
    public static Element createElement(RenderScript rs) {
        ...
        eb.add(Element.I8(rs), "bytes", 60);
        return eb.create();
    }

    private  ScriptField_char_array_60(RenderScript rs) {
        mItemArray = null;
        mIOBuffer = null;
        mElement = createElement(rs);
    }

    public  ScriptField_char_array_60(RenderScript rs, int count) {
        mItemArray = null;
        mIOBuffer = null;
        mElement = createElement(rs);
        init(rs, count);
    ...
    public  ScriptField_char_array_60(RenderScript rs, int count, int usages) {
        mItemArray = null;
        mIOBuffer = null;
        mElement = createElement(rs);
    ...
    public static ScriptField_char_array_60 create1D(RenderScript rs, int dimX, int usages) {
        ScriptField_char_array_60 obj = new ScriptField_char_array_60(rs);
        obj.mAllocation = Allocation.createSized(rs, obj.mElement, dimX, usages);
        return obj;
    }

    public static ScriptField_char_array_60 create1D(RenderScript rs, int dimX) {
        return create1D(rs, dimX, Allocation.USAGE_SCRIPT);
    }

    public static ScriptField_char_array_60 create2D(RenderScript rs, int dimX, int dimY) {
        return create2D(rs, dimX, dimY, Allocation.USAGE_SCRIPT);
    }

    public static ScriptField_char_array_60 create2D(RenderScript rs, int dimX, int dimY, int usages) {
        ScriptField_char_array_60 obj = new ScriptField_char_array_60(rs);
    ...
    public static ScriptField_char_array_60 createCustom(RenderScript rs, Type.Builder tb, int usages) {
        ScriptField_char_array_60 obj = new ScriptField_char_array_60(rs);
        Type t = tb.create();
    ...
    public synchronized void set_bytes(int index, byte[] v, boolean copyNow) {
        ...
        if (copyNow)  {
            mIOBuffer.reset(index * mElement.getBytesSize());
            for (int ct1 = 0; ct1 < 60; ct1++) {
                mIOBuffer.addI8(v[ct1]);
            }

            FieldPacker fp = new FieldPacker(60);
            for (int ct1 = 0; ct1 < 60; ct1++) {
                fp.addI8(v[ct1]);
            }
